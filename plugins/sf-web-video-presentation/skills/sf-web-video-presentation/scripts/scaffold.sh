...
#
# 例子：
#   bash <path-to-sf-web-video-presentation>/scripts/scaffold.sh ./presentation
#   bash <path-to-sf-web-video-presentation>/scripts/scaffold.sh ./talk --theme=paper-press
...
#
# 之后切换主题，覆盖一个文件即可：
#   cp <path-to-sf-web-video-presentation>/themes/<id>/tokens.css \
...
# create-vite 会把绝对路径参数误拼到 cwd 后面（path.join 行为），
# 所以先归一化成绝对路径，再 cd 到父目录用目录名调用，两种传法都安全。
...
echo "▸ 使用主题：$THEME"
(cd "$TARGET_PARENT" && npm create vite@latest "$(basename -- "$TARGET")" -- --template react-ts >/dev/null)
