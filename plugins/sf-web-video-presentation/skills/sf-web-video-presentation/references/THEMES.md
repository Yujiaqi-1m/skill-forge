...
内置 10 套精选主题，每个都有**独立的设计 DNA** —— 不是简单的换色版。
挑一个匹配你主题情绪的，或者作为你自己主题的起点。

**权威清单以 `themes/*/theme.json` 为准**（Checkpoint Plan 时动态读全部，
不要硬编码）；下面的性格速览只是帮助选型。

| id | 明暗 | 性格速览 |
|---|---|---|
| `midnight-press` | 暗 | 电影感编辑级深色。暖 espresso + 火热橙，斜体衬线 hero 数字，慢速节奏（脚手架默认） |
| `blueprint` | 暗 | 工程蓝图。深海军蓝 + 绘图青 + 等宽字体，虚线 rule + 制图网格是签名 |
| `terminal-green` | 暗 | 80 年代磷光终端。纯黑 + JetBrains Mono + CRT 扫描线，利落线性动效 |
| `chalk-garden` | 暗 | 深石板黑板。全场手写体 + 粉笔黄，虚线 rule + 粉笔字阴影 |
| `dark-botanical` | 暗 | 高级感编辑暗底，时尚刊物 / 博物馆图录。暖陶 / 玫粉 / 鎏金叠层，柔光晕染 |
| `paper-press` | 亮 | midnight-press 的白天孪生。暖奶油 + 纸纹 + 火热橙，杂志感 |
| `newsroom` | 亮 | NYT 报刊。报纸奶油 + 墨黑衬线 + 旗红，0 圆角，安静印刷节奏 |
| `bauhaus-bold` | 亮 | 现代主义宣言。米白 + 墨黑 + 原色蓝，0 圆角 + 实色厚边 + 偏移阴影 |
| `pastel-dream` | 亮 | 友好柔光。柔粉蓝灰 + 大圆角 + 多色 pill 色条，soft springy |
| `swiss-ikb` | 亮 | 瑞士国际主义。极细 Inter + 净暖白 + 克莱因蓝 + 发丝网格，直角 |
...
```bash
cp <path-to-sf-web-video-presentation>/themes/newsroom/tokens.css \
...
| 柔粉 / 友好 / onboarding                | `pastel-dream`        |
| 瑞士国际主义 / Helvetica / 信息驱动设计 | `swiss-ikb`           |

```bash
cd <path-to-sf-web-video-presentation>/themes
cp -r paper-press my-theme
```
...
主题放进 `themes/<id>/` 即被自动发现（`--list-themes` 与 Checkpoint Plan
都动态扫描目录）。可顺手在上表追加一行性格速览，但不是必须。
