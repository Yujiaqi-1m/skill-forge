---
name: sf-web-video-presentation
description: >
  把一篇文章或口播稿，做成"看起来像视频"的点击驱动 16:9 网页演示（Vite + React + TS），可选合成口播音频并自动录屏。流程：一次产出
  口播稿 + outline 开发计划 → 用户一次对齐 5 件事（稿子 / outline / 主题 / 素材 / 开发模式）→ 逐章开发（每章 narrations.ts 是 step 数与音频的唯一真相源）→
  可选音频合成（内置 MiniMax mmx-cli + OpenAI TTS，可换 ElevenLabs / edge-tts / Azure）→ 录屏。Trigger when the user says "把这篇文章做成视频",
  "帮我做一个视频网页", "动态 PPT", "录屏教程", "make this article into a video", "web presentation", "screen-recordable slides",
  或要做 B 站 / YouTube / 视频号解说视频、有电影感的产品 / talk demo。产出是可录屏的网页项目，不是视频文件；用户没给素材时不替用户从零构思内容。
license: MIT
compatibility: "Agent Skills standard; self-check protocol degrades from reviewer subagent to inline check; tested with Claude Code, Codex CLI, pi"
---
...
口播驱动的解说视频（B 站 / YouTube / 视频号）、教学 / 产品演示 /keynote、想
要"动态 PPT 但不像 PPT"的 16:9 横屏录屏。

本 Skill **以方法论 + 协作流程为核心**。脚手架模板提供 token 和原语，但每个
美学决策（配色、字型、动效气质）都应该针对你的主题重新设计 ——不要照搬。
...
1. **独立 reviewer 子 agent（最优）**：开一个独立的 reviewer 子 agent，
   给它"产出文件路径 + 对应清单 + 关键上下文"，让它逐项核查并   **严格汇报结
论**（哪几条 pass / 哪几条 fail + 证据 + 改写建议）。
2. **自检（兜底）**：当前 agent 开不了子 agent，就自己**严格逐项**
   核查 —— 不允许目测一遍就放行。

**铁律**：拿到结论后**先按 fail 项把产出改完**，再向用户汇报"做完了+ 自检结
论 + 改了什么"。**直接拿原始结论汇报但不修复 = 违规**。

---

## 各阶段文件读取指南

不同阶段读不同的文件。**长会话里 agent 容易遗忘原则**，特别是Phase 2.4 的"实
现单章"会重复 N 次 —— 每次都要回看核心约束。
...
| **Phase 2.4 实现单章（×N 次，被 2.2 / 2.3 调用）** | **`references/CHAPTER-CRAFT.md`** 单一入口 —— Part 0 十条原则 / Part 1 开工 5 问 / Part 2 关系→动作决策树 / Part 3 视觉工具箱 / Part 4 时长参考 / Part 5 反 AI 味反模式 / Part 6 代码硬规则（**含 narrations.ts 强制约束**）/ Part 7 完工自检 / Part 8 反馈速查 + 当前主题的 `themes/<id>/theme.json` + 当前章节的 outline.md 段落 + **`article.md` 本章对应段落** + 素材清单 | `references/THEMES.md` 完整 token 契约 |
...
1. **生成 `script.md`**：按 [`references/SCRIPT-STYLE.md`](references/SCRIPT-STYLE.md)
   的规则把 article 转成保持原文语言的平台化口播稿。**保留 `article.md` 不删
**——它是   outline 写信息池和章节实现画面时的细节源（双源原则）。
...
**落盘后必须先走自检再进 Checkpoint Plan**：按上文「硬性自检协议」分别对
 `script.md` / `outline.md` 执行（优先独立 reviewer 子 agent → 自检），按结
论修复完成后再进入 Checkpoint Plan。

---

## Checkpoint Plan —— 5 件事一次对齐（**硬节点**）

`script.md` + `outline.md` 写完后必须停下来。**用户在这一个节点同时确认5 件
事**。
...
```bash
bash <path-to-sf-web-video-presentation>/scripts/scaffold.sh \
  ./presentation \
  --theme=<用户选的主题 id>

bash <path-to-sf-web-video-presentation>/scripts/scaffold.sh --list-themes
```
...
并把 `presentation/src/registry/chapters.ts` 里 `EXAMPLE_CHAPTER`的 import 
和数组项移除。

### 2.2 第 1 章 —— 主线程 + 强制验收

**核心**：第 1 章 = 完整版本一次到位（节奏 + 视觉 + 真素材齐全）。**没有"骨
架版"概念** —— 第一章就要做出**用户能直接验收**的样板。

为什么第 1 章必须主线程：

- 它是 [`CHAPTER-CRAFT.md`](references/CHAPTER-CRAFT.md) 指引在**当前主题 +
  当前题材**下的第一次落地 —— 指引盲区 / token 缺口一定在这里暴露，  有人类
反馈就能早修，**成本最低**
- 后续章节（无论顺序 / 并行）都参考第 1 章的代码模式，所以第 1 章 =
...
**所有模式下的共同规则**：每章独立按
 [`CHAPTER-CRAFT.md`](references/CHAPTER-CRAFT.md)开发。**风格不强求章节间完
全一致** —— 主题颜色 / 字体 token 兜底视觉统一，动画 / 节奏 / 视觉演示由章节
自由发挥是设计预期。

#### 模式 A · 默认 · 逐章确认

第 2 章做完 → 暂停验收 → OK → 第 3 章 → 暂停 → ... → 第 N 章。**每章独
立验收**，问题随时改，**风险最低，节奏最稳**。**用户不明确选模式时默认走这个
**。

#### 模式 B · 第 1 章后顺序开发

第 2 章 → 第 3 章 → ... → 第 N 章 **主线程顺序做完，最后统一验收**。速度
中等，适合 agent 不支持并行任务的环境。

#### 模式 C · 第 1 章后并行开发（subagent）

用 subagent 把第 2~N 章并行做完，最大并行数由用户控制（"一次 4 章"/ "一次 2 
章"）。**最快，但风格各章会有差异 —— 这是预期**：每个subagent 看不到彼此产出
，无法机械对齐；章节代码物理分离（每章一个文件夹 / 自己的 CSS 前缀）互不破坏
；主题 token 兜底视觉统一，气质不会跑偏；风格不一致本身 = 人手写视频的呼吸感
（多 voice / 多视角）。
...
**重要**：无论选哪种模式，**用户随时可以中途切换模式**。第 2 章 OK后用户说"
剩下的并行" / "剩下的逐章" 都行。

### 2.4 实现单章（每章必走）

详细指引见 [`references/CHAPTER-CRAFT.md`](references/CHAPTER-CRAFT.md) ——**
单一必读入口**，覆盖：视觉演示要求 / 逐步揭示 / 内容取舍 / 双源原则/ 视频演
示基本审美 / 反 AI 味 / 代码红线 / 完工自检。
...
- **完工自检逐项过**，不达标回去改 —— 按上文「硬性自检协议」执行
  （优先独立 reviewer 子 agent → 自检），**改完再向用户汇报本章交付**

### 2.5 大改后 bump STORAGE_KEY

改动 `chapters.ts`（增加 / 删除 / 重排章节，或某章 `narrations.ts`长度变化）
后，**bump** `presentation/src/hooks/useStepper.ts` 的`STORAGE_KEY`（如
 `v4` → `v5`），避免持久化游标落到不存在的 step 上。
...
合成完告诉用户：输出位置 / 总段数 / 哪些段时长异常（太长 = 该 step 拆分；太
短 = 文案太薄）—— 给最后一次校准节奏的机会。然后进入 Phase 4。
...
完整展开见
 [`references/CHAPTER-CRAFT.md`](references/CHAPTER-CRAFT.md)Part 0 —— **写
章节时回那里查**，下面只是索引。
...
简化表见 [`references/CHAPTER-CRAFT.md`](references/CHAPTER-CRAFT.md)Part 8
「常见反馈速查」。**关键**：先定位是哪一层（节奏 / 视觉 / 内容/ 代码），再改
最小切片，**不要重做整章**。
...
> 本 skill 蒸馏自 [ConardLi/garden-skills](https://github.com/ConardLi/garden-skills) 的
> `web-video-presentation` v1.2.2（MIT），方法论、协作流程与模板结构保留原作；
> 精选了其中的 10 套主题。
